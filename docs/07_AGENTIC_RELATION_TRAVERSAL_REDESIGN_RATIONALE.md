...
이 문서는 현재 수페스타가 관계 질문을 처리하는 실제 방식을 과장 없이 규정하고, 이를 `AI가 한 단계씩 지식을 관찰하고 다음 관계를 선택하는 방식`으로 개편할 때 생기는 기술적·개념적·시연적 의미를 판단하기 위해 작성한다.
...
현재 경로는 질문별로 하드코딩된 경로가 아니다. Local Qwen은 관계를 조사할 목표와 도구를 선택하지만, 중간 노드와 edge의 연속 경로는 그래프의 `shortest_path()` 알고리즘이 한 번에 계산한다. 따라서 현재 구조를 `AI가 중간 지식을 하나씩 판단해 스스로 경로를 만들었다`고 설명하면 과장이다.

단계별 관계 추적으로 개편하면, 각 지식 이동이 `AI Action → CCS Observation`으로 분리된다. 이때부터 AI가 현재 관찰한 지식을 바탕으로 다음 지식을 선택하고, 그 선택의 연속으로 경로를 구성했다고 말할 수 있다.

다만 정확성과 속도만 보면 현재 알고리즘 방식이 더 유리할 수 있다. 본 프로젝트처럼 `지식이 AI의 다음 행동을 발생시키는 과정` 자체를 증명하려는 경우에는 단계별 개편의 의미가 크다.

> **최종 판단:** AI가 1-hop 관계를 선택하고, 결정론적 그래프 알고리즘은 답을 제공하는 탐색기가 아니라 사후 검증기와 안전장치로 남는 하이브리드 구조가 가장 타당하다.
...
현재도 벡터 RAG나 고정 답변 mock은 아니다. 또한 질문별 정답 route map도 없다. 그러나 경로의 중간 선택까지 AI 행동이라고 주장할 수는 없다.
...
현재 방식은 관계 Action 한 번에 전체 경로가 Observation으로 들어온다.
...
개편 후에도 AI가 새로운 ESG 지식을 창조하는 것은 아니다. CCS에 존재하는 지식 중 질문에 필요한 관계를 선택하여 경로를 구성한다.
...
단계별 추적은 현재 방식보다 자동으로 더 정확한 것이 아니다. 목표가 빠른 정답 제공뿐이라면 결정론적 최단 경로가 더 효율적일 수 있다.
...
- [ ] 전체 경로가 사전에 Qwen에게 제공되지 않는다.
- [ ] Qwen은 매 turn 현재 node의 1-hop 관계만 본다.
- [ ] 각 node 이동마다 독립 Action과 Observation이 기록된다.
- [ ] 선택한 edge는 실제 CCS graph에 존재한다.
- [ ] 최종 경로는 Action log로 재구성할 수 있다.
- [ ] 관찰하지 않은 edge와 node는 최종 claim에 사용할 수 없다.
- [ ] 경로가 없으면 환각하지 않고 backtrack 또는 STOP한다.
- [ ] Skill 선택은 실제 traversal 결과와 연결된다.
- [ ] 결정론적 경로 알고리즘은 탐색기가 아니라 사후 검증기로만 사용된다.
- [ ] 관계 순서 변경·방해 관계·역방향·edge 제거 시험을 통과한다.
- [ ] Local Qwen model identity와 모든 도구 호출이 run evidence로 보존된다.
- [ ] 인터넷과 원격 LLM 없이 동일 구조를 재현한다.
